...
def main() -> None:
    try:
        ...
        # Run retrieval
        logger.info("Running HybridRetriever to fetch top 10 candidates...")
        ...
        for i in range(len(ranked_results) - 1):
            assert ranked_results[i].features.final_score >= ranked_results[i+1].features.final_score, "Results are not sorted by final_score!"

        # Print results
        print("\n--- Final Ranked Candidates (Top 10) ---")
        for rank, result in enumerate(ranked_results, start=1):
            print(f"Rank: {rank}")
            print(f"candidate_id: {result.candidate.candidate_id}")
            print(f"hybrid retrieval score: {result.relevance_score:.6f}")
            print(f"final_score: {result.features.final_score:.6f}")
            print("-" * 42)
        print()
        
        logger.info("Ranking engine test completed successfully!")
